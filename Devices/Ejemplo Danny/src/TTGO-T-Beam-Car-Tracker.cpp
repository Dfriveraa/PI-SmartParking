...
static PROGMEM u1_t NWKSKEY[16] = { 0x9C, 0x53, 0x36, 0xC2, 0x12, 0xB1, 0xC9, 0x2A, 0xEB, 0xC4, 0x48, 0xFF, 0xAA, 0xBB, 0xEA, 0x1F }; // LoRaWAN NwkSKey, network session key 
static u1_t PROGMEM APPSKEY[16] = { 0xA3, 0xEA, 0x6D, 0x3A, 0x0E, 0x2C, 0x66, 0xDA, 0x38, 0x46, 0x4A, 0xFB, 0x21, 0x31, 0xEA, 0xAC }; // LoRaWAN AppSKey, application session key 
static const u4_t DEVADDR = 0x26031786; // LoRaWAN end-device address (DevAddr)
...
#ifdef DEBUG
const unsigned int TX_INTERVAL = 10;
#elif
...
void do_send(osjob_t* j) {  
  ...
  { 
    if (gps1.checkGpsFix())
    {
      ...
      Serial.println(F(" bytes long LPP packet queued whith gps"));
